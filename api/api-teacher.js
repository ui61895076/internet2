...
router.post('/register',(req,res)=>{
    ...
    let sql2 =`select * from teacher where name = '${req.body.name}'`
...
router.post('/login',(req,res)=>{
    let sql = `select * from teacher where name = '${req.body.name}' and pwd= '${req.body.pwd}'`
...
router.post('/filterUser',(req,res)=>{
    let sql=`select * from teacher where name='${req.body.name}'`
    connection.query(sql,(err,result)=>{
        if(err) throw err
        if(result.length==0){
...
router.post('/restpwd',(req,res)=>{
   // console.log(filterUser_name)
    let sql2 =`update teacher set ? where name= '${filterUser_name}'` //需要更新的用户的信息
